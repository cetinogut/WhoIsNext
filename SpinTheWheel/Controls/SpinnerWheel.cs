...
namespace SpinTheWheel.Controls
{
    public partial class SpinnerWheel : UserControl
    {
        ...
        //public List<Student> Students { get; set; } //wheel daki liste in class olması gerektiği için ismini aşağıdaki gibi değiştirdim.
        public List<Student> StudentsReadyInClass { get; set; }
        public bool AnimationEnabled { get; set; } = true;
        public bool StudentReelectionInSession { get; set; } = false;
        ...
        //    selectedIndex = random.Next(0, StudentsReadyInClass.Count);

        //    step = (float)360 / (float)StudentsReadyInClass.Count;
        ...
        private void nonFlickerPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (StudentsReadyInClass != null)
            {
                var random = new Random();
                float studentNumber = StudentsReadyInClass.Count;
                ...
                if (spinnerBitmap == null)
                {
                    ...
                    for (int i = 0; i < studentNumber; i++)
                    {
                        ...
                        studentDisplayName += $"{StudentsReadyInClass[i].FirstName} {StudentsReadyInClass[i].LastName.ToCharArray()[0]}. {StudentsReadyInClass[i].Number}";
        ...
        private void BtnRoundSpin_Click(object sender, EventArgs e)
        {
            BtnRoundSpin.Enabled = false;
            Application.DoEvents();
